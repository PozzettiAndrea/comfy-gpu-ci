...
CPU tests run per-repo on GitHub-hosted runners (free). GPU tests run here on self-hosted runners with NVIDIA GPUs, inside Docker containers for isolation.

## How it works

1. Dashboard or manual dispatch triggers `test-gpu.yml` with `node_repo`, `branch`, and `platform`
2. Self-hosted runner picks up the job
3. `comfy-test run --gpu --publish` runs inside a Docker container with GPU passthrough
...
- `windows-gpu` — Windows container with NVIDIA GPU passthrough (`--isolation=process --device class/...`)
- `linux-gpu` — Linux container with `--gpus all`
- `windows-portable-gpu` — Portable ComfyUI build on Windows GPU
